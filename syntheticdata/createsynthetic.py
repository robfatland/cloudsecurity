# 100 people (all named John) live in a small town. They appeared in this town rather mysteriously on
#   October 30, 1938. Every day or two one of these people, selected at random, visits the doctor
#   where their vital signs are recorded: Blood pressure (two numbers), respiration rate, heart rate,
#   blood oxygen saturation, body temperature (degrees F) and weight (pounds). The doctor also asks
#   'Since your last visit how many Count Basie albums have you purchased? Albums by Sarah Vaughan?
#   Albums by Frank Zappa? This code produces a 10,000 row time-series dataset spanning 41 years. 
#
# Three tables are constructed and written to CSV files.
#
#  Tstudy is the first day of study data, the morning of October 30 1938
#  Tborn is the latest possible date of birth, Dec 31 1922 (All participants are 16 or older)
#  All participants give their DOB as August 26 1920.
#
# The Python random number generator is given a fixed seed so that this code always generates the 
#   same patient history.
#
# Output files:
#  patients.csv:          patient table p: Surname, Given Name, DOB, (height feet, inches), patient ID
#  patientParameters.csv: patient parameters pp: for internal use
#  timeseries.csv:        time series data ts: Across all patients
#
...
def RandLastName():
    '''
    Some surnames are just random letter sequences
    '''
    return uc[r.randint(0,25)]+lc[r.randint(0,25)]+lc[r.randint(0,25)]+lc[r.randint(0,25)]

def cmtoftin(ncm):
    '''
    convert patient height in cm to feet-inches
    '''
...
def ftintocm(height):
    '''
    feet-inches to centimeters for patient height
    '''
...
def someheight():
    '''
    randome height of patient in cm
    '''
    hgtLow = 157
...
tsHdr.append(['date','ID','systol','diastol','resp.rate','heart rate','OSat','temp',\
           'weight','BMI','Basie','Vaughan','Zappa','sum Basie','sum Vaughan','sum Zappa'])
...
for i in range(10000):
    ...
    thisBaseie = ModelBaseie(thisHR, thisSys, thisDia, thisBMI)
    thisVaughan = ModelVaughan(thisRR, thisOS, p[thisID][pHeightIndex])
    thisZappa = ModelZappa(thisBMI, thisBT)
    
    # track the cumulatives of the diagnostics
    pp[thisID][ppSUMBidx]+=thisBasie
    pp[thisID][ppSUMTidx]+=thisVaughan
    pp[thisID][ppSUMDKidx]+=thisZappa
    thisSumBasie = pp[thisID][ppSUMBidx]
    thisSumVaughan = pp[thisID][ppSUMTidx]
    thisSumZappa = pp[thisID][ppSUMDKidx]

    # create a new record in the time series
    ts.append([Time, thisID, thisSys, thisDia, thisRR, thisHR, thisOS, \
               thisBT, thisMass, thisBMI, thisBaseie, thisVaughan, thisZappa, \
               thisSumBasie, thisSumVaughan, thisSumZappa])
